...
data_main10_groups |> 
  left_join(rename(data_ggdc_level05, levels05 = values)) |> 
  group_by(geo, nace0) |> 
  ...
  filter(geo %in% geo_level,
         nace0 == "private") |> 
  ggplot(aes(time, lp_level05, colour = geo_nimi)) +
  geom_line() +
  scale_y_log10()

save_figs("level_private", en = FALSE)
...
save_figs("level_service", en = FALSE)
...
data_main10_groups |> 
  ...
  mutate(lp_level05 = levels05 * lp_ind) |> 
  group_by(time, nace0) |> 
  mutate(lp_level05_rel_us = 100 * lp_level05 /lp_level05[geo == "US"]) |> 
  ungroup() |> 
  filter(geo %in% geo_level,
         nace0 == "private") |> 
  ggplot(aes(time, lp_level05_rel_us, colour = geo_nimi)) +
  geom_line() +
  scale_y_log10()
```
