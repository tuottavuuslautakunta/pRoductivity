...
dat_kuviot_johd[c("1.5A", "1.5B", "1.5C")] %>% #str() # names()
  ...
  # mutate(geo = fct_reorder(Maa, keskiarvo)) %>% 
  ggplot(aes(reorder(V4, keskiarvo), keskiarvo, fill = Maa == "Suomi")) +
...
save_figs("Kuvio_1_5_III", height = fig_height +2)
...
save_figs("Kuvio_1_6_III", height = fig_height +2)
...
dat_kuviot_johd[["1.12"]] %>% # names()
  ...
  geom_hline(yintercept = 100) +
  labs(y = "%",
       title = unique(dat_kuviot_johd[["1.12"]]$otsikko)) +
  ylim(c(25,175)) 
