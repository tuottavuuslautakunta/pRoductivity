...
data_luiss_groups_tfp2 |> 
  ggplot(aes(time, tfp, colour = geo)) +
  geom_line()
```

```{r}

bind_rows(data_luiss_groups_tfp |> 
  filter(vars == "tfp",
         nace0 == "private") |> 
    group_by(geo) |> 
    mutate(values = rebase(values, time, 1995)) |> 
    ungroup(),
# data_luiss_groups_tfp2 |> 
#   rename(values = tfp) |> 
#   mutate(vars = "ftp2"),
dat_klems_luiss_ext |> 
  ...
  group_by(geo, nace_r2, vars) |> 
  mutate(values = 100 * values / values[time == 2008]) |> 
  ungroup() |> 
...
data_luiss_groups_tfp |>
  filter(vars == "tfp",
         nace0 %in% c("private", "private_ex26")) |> 
   filter_recode(geo = geo_main2) |> 
    group_by(geo, nace0, vars) |> 
  mutate(values = 100 * values / values[time == 2008]) |> 
  ungroup() |> 
