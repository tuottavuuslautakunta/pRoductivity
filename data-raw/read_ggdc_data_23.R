...
ggdc_file_23 <- here::here("data-raw", "pld2023_dataset.xlsx")

dat0_ggdc_23 <- readxl::read_xlsx(ggdc_file_23, sheet = "Data")

dat_ggdc_23 <-
  dat0_ggdc_23 |>
  statfitools::clean_names(to_lower = TRUE) |>
  mutate(lp = (va / ppp_va) / emp) |>
  mutate(lp_xr = (va / xr) / emp) |>
  group_by(year, sector) |>
  mutate(lp_level = lp / lp[countrycode == "USA"]) |>
...
data_ggdc_level05 <- readxl::read_xlsx(ggdc_file, sheet = "LP_GDP_MajorSector", skip = 1) |>
  select(-...4) |>
  statfitools::clean_names(to_lower = TRUE) |>
  mutate(geo = countrycode::countrycode(iso_code, "iso3c",
                                        destination = "eurostat",
                                        custom_match = c(EU27 = "EU27", EURO17 = "EA12")),
  ...
  filter(geo %in% c(countries, "EA12"))

data_ggdc_va05 <- readxl::read_xlsx(ggdc_file, sheet = "VA_GDP_MajorSector", skip = 1) |>
  select(-...4) |>
  statfitools::clean_names(to_lower = TRUE) |>
  mutate(geo = countrycode::countrycode(iso_code, "iso3c",
                                        destination = "eurostat",
