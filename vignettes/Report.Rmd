...
```{r private}
data <- ssca_lp_ind$model$private$z_list$theCall$longdata

p1 <- ssca10_lp_ind$model$private$z_list$theCall$longdata %>% 
  filter(time >= 2000) %>% 
...
data %>% distinct(geo, geo_name)  
  ...
  distinct(geo, geo_name, high_names)
